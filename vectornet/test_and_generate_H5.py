...
global_step = 0
# FILENAME = 'epoch_24.valminade_2.500.200707.epochs50.lr_decay0.3.decay_every15.lr0.0003.xkhuang.pth'
FILENAME = 'epoch_24.valminade_12.635.200621.xkhuang.pth'
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            data = data.to(device)
            out = model(data)
            gt = data.y.reshape(out.shape[0],-1).cpu()
            for i in range(out.size(0)):
                ...
                file = open(f"./visualize/pred_and_gt/agent{i}_pred_gt.jpg","a")
                file.close()
                plt.savefig(f"./visualize/pred_and_gt/agent{i}_pred_gt.jpg")
