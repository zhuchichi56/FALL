...
def get_eval_metric_results(model, data_loader, device, out_channels, max_n_guesses, horizon, miss_threshold,whether_plot):
    """
    ...
    with torch.no_grad():
        for data in data_loader:
            ...
            for i in range(gt.size(0)):
                ...
                seq_id += 1
        if whether_plot is True:
            for i in range(seq_id):
                for n_th_predict in range(len(forecasted_trajectories[i])):
                    show_pred_and_gt(forecasted_trajectories[i][n_th_predict],gt_trajectories[i])
                    plt.savefig(f"./visualize/pred_and_gt/{i}_{n_th_predict}-th_prediction_pred_and_gt.jpg")
