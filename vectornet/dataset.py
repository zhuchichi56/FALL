...
import torch
from utils.config import DATA_DIR, INTERMEDIATE_DATA_DIR
...
class GraphData(Data):
    ...
    def __inc__(self, key, value, *args, **kwargs): # 可以试试没有后面2个参数会有非常奇怪的错误
        if key == 'edge_index':
...
class GraphDataset(InMemoryDataset):
    ...
    def __init__(self, root, max_load_num, transform=None, pre_transform=None, specific_index=None):
        self.max_load_num = max_load_num  # 这个构造器的顺序很重要啊
    ...
    def process(self):
        ...
        if self.specific_index is not None:
            data_path_ls = [data_path_ls[self.specific_index]]
            print(f"Graph dataset read pkl  from {data_path_ls}")
        else:
            if self.max_load_num > 0:  # 如果是-1代表取全部
                data_path_ls = data_path_ls[:self.max_load_num]
        ...
        for data_p in tqdm(data_path_ls):
            ...
            cluster = all_in_features[:, -1].reshape(-1).astype(np.int32) # 这个是指示哪些点要放在去生成这个subgraph
            valid_len_ls.append(cluster.max()) # 这个就是指的是我这个场景有多少需要注意的
            ...
            for id_, mask_ in traj_mask.items():
                ...
                edge_index_, edge_index_start = get_fc_edge_index(
                    data_.shape[0], start=edge_index_start) # 这一部分就是经典的构图
                x_ls.append(data_)
